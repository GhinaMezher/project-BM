...
dep_pub|>
  summarise("Origin"="World bank","Last update"= dep_pub_raw[[1,2]],
...
ggplot(mean_by_date,aes(x=time,y =mean_ps)) +
  geom_col() +
  ylim(0,7)+
  labs(x = "Date", y = "World mean Public Spending (% of GDP)", title = "Mean Public Spending Over Time")+
...
ggplot(mean_g8,aes(x=time,y=mean_public_spending))+
  geom_point(na.rm = FALSE) +
   geom_line(na.rm=FALSE)+
    labs(title = "Public spending on education trends for developped",
       x = "Time",
...
dep_chomage_afr <- dep_chomage |>
  filter(!is.na(`Public Spending (% of GDP)`))|>
  filter(Country %in% c("MDG", "MLI", "NER", "MWI","RWA", "SSD", "ZMB","DJI","AFE", "BDI", "BFA", "BLZ", "CAF", "COD", "LBR", "SDN", "SEN", "DAZ", "BWA", "TCD", "EGY", "ERI", "ETH", "GMB", "KEN", "LBY", "MAR", "NAM", "NGA", "SLE", "ZAF", "TGO", "TUN", "YEM", "ZWE"))
...
ggplot(dep_chomage_afr_filtered, aes(x = time, y = `Public Spending (% of GDP)`, color = Country, group = Country, shape = Country)) +
  geom_line(na.rm = FALSE) +  
  geom_text(aes(label = Country), hjust = 0, vjust = 0, size = 3, color = "black")+
...
```{r}
dep_chomage_g8_filtered <- dep_chomage_g8_filtered |>
  mutate(quintile = ntile(`Public Spending (% of GDP)`, 5))

print(dep_chomage_g8_filtered)
```
```{r}
ggplot(dep_chomage_g8_filtered, aes(x = factor(quintile), y = `Youth unemployment (%)`)) +
  geom_boxplot() +
  labs(title = "Distribution of Youth Unemployment Across Quintiles of Public Spending",
       x = "Quintiles of Public Spending",
       y = "Youth Unemployment (%)") +
  theme_minimal()
```
...
### Linear regression for G8 countries 

```{r}
linear_model <- lm(`Youth unemployment (%)` ~ `Public Spending (% of GDP)` + `Education`, data = dep_chomage_g8_filtered)
