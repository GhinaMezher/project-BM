...
population_world|>
  summarise("Origins"=table_pop[1],
            ...
            "Nb of countries"= length(unique((population_world$ref_area.label))),
            "Nb of sex"= length(unique((population_world$sex.label))),
            "Age groups "= length(unique((population_world$classif1.label))),
...
colnames(population_world)[colnames(population_world) == "ref_area.label"] <- "Country"
colnames(population_world)[colnames(population_world) == "sex.label"] <- "Sex"
colnames(population_world)[colnames(population_world) == "classif1.label"] <- "Age groups"
colnames(population_world)[colnames(population_world) == "classif2.label"] <- "Attendance"
...
population_world <- population_world |> 
  mutate(`Age groups`=case_when(`Age groups`=="Age (Tranches des jeunes): 15-19" ~ "15-19",
                                `Age groups`=="Age (Tranches des jeunes): 15-29" ~ "15-29",
                                `Age groups`=="Age (Tranches des jeunes): 20-24" ~ "20-24",
                                `Age groups`=="Age (Tranches des jeunes): 25-29" ~ "25-29",
         TRUE ~`Age groups`))
chomage_jeune<-chomage_jeune|>
  mutate(`sex`=case_when(`sex`=="SEX_M"~"Male",
                       `sex`=="SEX_F"~"Female",
...
# Cleaning the ILO database 
#We remove some column which will not used for the projet like "Source", "Indicator", "obs_status", "note_indicator", "note_source", and we create a new database called chomage_jeune_clean.These are just codes used by the ILO to classify data.
population_world_cleaning<-function(df){
  single_value<-function(my_var){
  (my_var|>unique()|>length())==1
  }
  df_clean<-df|>
    select(!where(\(.x)all(is.na(.x))))|>
    select(!where(single_value)) |>
    select(-"source.label", -"obs_status.label", -"note_source.label", -"note_indicator.label", -"note_classif.label")
}
pop_clean<-population_world_cleaning(population_world)
```


```{r}
# Assuming 'Country' is the common column
merged_data1 <- merge(dep_pub_clean, pop_clean, by = "Country", all = TRUE)

# Assuming 'Country' is the common column
merged_data2 <- merge(chomage_jeune_clean, pop_clean, by = "Country", all = TRUE)
```

```{r}
# Assuming 'Country' is the common column
merged_data <- merge(merge(dep_pub_clean, pop_clean, by = "Country", all = TRUE), chomage_jeune_clean, by = "Country", all = TRUE)
```
