...
ggplot(dep_chom_14_22, aes(x = time, y = `Youth unemployment (%)`)) +
  ...
  labs(title = "Figure 6 :Youth Unemployment Trends (2014-2022)",
       ...
       caption = "Source: ILO")
```
Through this graph we can see countries with highest youth unemployment rate at the top of the line. 
...
mean_dep<-dep_chomage|>
  filter(time==2021)|>
  mutate(dummy=ifelse(dep_chomage$`Public Spending (% of GDP)`>= mean(`Public spending (% of GDP)`), 1, 0))

ggplot(dep_chomage, aes(x = mean_dep,y=`Youth unemployment (%)`,fill = Education)) +
  geom_bar()
```
