...
ggplot(dep_chomage, aes(x = time, y = `Public Spending`)) +
...
We'll be looking at public spending trends in developed countries such as the Afghanistan, Angola, Bangladesh, Bénin, Bhoutan, Burkina Faso, Burundi, Cambodge, Tchad, Comores, Djibouti, Erythrée, Ethiopie, Gambie, Guinée, Guinée-Bissau, Haiti, Iles Salomons, Kiribati, Lesotho, Libéria, Madagascar, Malawi, Mali, Mauritanie, Mozambique, Myanmar, Népal, Niger, Rwanda, Sao Tome et Principe, Sénégal, etc.. 

```{r}
dep_by_country <- dep_chomage %>%
  filter(!is.na(time), !is.na(`Public Spending`),
         time >= 2014 & time <= 2022,
         Country %in% c("AFG", "AGO", "ALG", "ANG", "ARG", "AZE", "TCH", "LES", "MAD", "GAM", "SEN", "NIG", "BUR", "MAU", "NEP", "CAM", "BRA"))
...
```{r}
correlation_result <- cor.test(dep_chomage$`Public Spending`, dep_chomage$`obs_value`)
correlation_result
```
...
```{r}
ggplot(dep_chomage, aes(x = `Public Spending`, y = `obs_value`)) +
  geom_point() +
...
```{r}
library(tidyverse)
regression_model <- lm(`obs_value` ~ `Public Spending`, data = dep_chomage)
summary(regression_model)
```
```{r}
ggplot(dep_chomage, aes(x = `obs_value` , y = `Public Spending`)) +
  geom_point() +
