...
chomage_jeune|>
  summarise("Origin"=name_col[1],
            ...
            "Nb of sex"= nrow(chomage_jeune)[4],
...
```{r}
#We rename the culumn to better understand the variables
...
colnames(chomage_jeune)[colnames(chomage_jeune) == "classif2"] <- "Student situation"
```
...
```{r}
dep_pub|> count(`Country Name`, `Country Code`)
dep_pub_clean<-dep_pub|>select(-`Country Name`)
```
...
```{r}
# We remove some column which will not used for the projet like "Source", "Indicator", "obs_status", "note_indicator", "note_source"

chomage_jeune_1 <- chomage_jeune %>% select(-"indicator", -"source", -"obs_status", -"note_source", -"note_indicator")
```

### Data joining 

```{r}
dep_pub<- data.frame(ID = c(1, 2, 3), Nom = c("Country Code", "Indicator name", "2022"))
chomage_jeune <- data.frame(ID = c(4, 5, 6, 7, 8), Nom = c("ref_areas", "Age groups", "Students situation", "time", "obs_value"))

result <- bind_rows(dep_pub, chomage_jeune)
```
