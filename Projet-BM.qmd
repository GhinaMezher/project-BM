...
```{r}
dep_chomage <- dep_chomage %>%
  filter(!is.na(time), !is.na(`Public Spending (% of GDP)`),
         time >= 2014 & time <= 2022)

ggplot(dep_chomage, aes(x = time, y = `Public Spending (% of GDP)`)) +
...
```{r}
dep_by_country <- dep_chomage %>%
  filter(!is.na(time), !is.na(`Public Spending (% of GDP)`),
         time >= 2014 & time <= 2022,
         Country %in% c("USA", "FRA", "GER", "JAP", "CAN", "GBR", "RUS", "ITA"))

ggplot(dep_by_country, aes(x = time, y = `Public Spending (% of GDP)`, color = Country, group = Country, shape = Country)) +
  geom_line() +  
  geom_point() + 
  labs(title = "Public spending on education trends for G8 countries (2014-2022)",
       x = "Time",
       y = "Public spending (% of GDP)",
       color = "Country") + 
  theme_minimal() 
...
```{r}
dep_by_country_africa <- dep_chomage%>%
  filter(!is.na(time), !is.na(`Public Spending (% of GDP)`),
         time >= 2014 & time <= 2022,
         Country %in% c("AFE", "BDI", "BFA", "BLZ", "CAF", "COD"))

ggplot(dep_by_country_africa, aes(x = time, y = `Public Spending (% of GDP)`, color = Country, group = Country, shape = Country)) +
  geom_line() +  
  geom_point() + 
  labs(title = "Public spending on education trends for African countries (2014-2022)",
       x = "Time",
       y = "Public spending",
       color = "Country") + 
  theme_minimal() 
...
### Unemployement rate in the developped countries 
