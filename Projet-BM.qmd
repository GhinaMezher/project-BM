...
population_world<- vroom(here("pop.csv"))
```
...
We're interested in every country in the world, which explain why we haven't included OECD data, which is limited to 38 countries.
...
### A table summary of our ILO database : unemployement 
...
colnames(chomage_jeune)[colnames(chomage_jeune) == "obs_value"] <- "Youth unemployement"
...
### A table summary of our ILO database : young population 

```{r}
# Our database chomage_jeune contains a huge number of observations with 11 variables, so we'll remove the missing NA values to reduce the number of observations.

table_pop<-colnames(population_world)
population_world|>
  summarise("Origins"=table_pop[1],
            "Nb of rows"=n(),
            "Nb of columns"=ncol(population_world),
            "Nb of countries"= length(unique((population_world$ref_area))),
            "Nb of sex"= length(unique((population_world$sex))),
            "Age groups "= length(unique((population_world$classif1))),
            "Time " = length(unique((population_world$time))) 
            )|>
  knitr::kable()
```
...
colnames(population_world)[colnames(population_world) == "sex.label"] <- "sex"
colnames(population_world)[colnames(population_world) == "classif1"] <- "Age groups"
colnames(population_world)[colnames(population_world) == "obs_value"] <- "Young people of working age"
...
```{r}
# Cleaning the ILO database
#We remove some column which will not used for the projet like "Source", "Attendance", "Indicator", "obs_status", "note_indicator", "note_source", and we create a new database called pop_clean.These are just codes used by the ILO to classify data.
pop_cleaning<-function(df){
  ...
  df_clean<-df|>
    ...
    select(!where(\(.x)all(is.na(.x))))|>
    select(!where(single_value)) |>
    select(-"source", -"classif2", -"obs_status", -"note_source", -"note_indicator")
}
pop_clean<-pop_cleaning(population_world)
```

### Merge our three database 
```{r}
# Merge chomage_jeune_clean and pop_clean to have only one database from ILO
chomage_pop <- merge(chomage_jeune_clean, pop_clean, by = c("Country", "time", "sex", "Age groups"), all = TRUE)

# Merge the new database from ILO with dep_pub_clean 
dep_chomage_pop <- merge(chomage_pop, dep_pub_clean, by = c("Country", "time"), all = TRUE)

# We have a new database merging by three database. In the following we will maily use it. 
``` 
...
Applying a ratio can be useful for comparing levels of youth unemployment between countries, because there are significant differences in the way countries count youth participation in the workforce.

### Unemployement rate in developped countries 

We'll add a new column called "unemployment rate", calculated as the ratio of unemployed young people to the total number of young people. 

```{r}
# We use mutate and we calculate the new column. 
dep_chomage_pop_1 <- dep_chomage_pop %>%
  mutate(`Young Unemployment Rate` = (`Youth unemployement` / `Young people of working age`)*100)
```

```{r}
France <- dep_chomage_pop_1 %>%
  filter(Country == "FRA")

ggplot(France, aes(x = time, y = `Young Unemployment Rate`)) +
  geom_line() +
  labs(title = "Évolution du taux de chômage des jeunes en Afghanistan (2014-2022)",
       x = "Année",
       y = "Taux de chômage des jeunes (%)") +
  theme_minimal()
```
