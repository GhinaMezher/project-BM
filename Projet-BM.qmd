...
## I. The importance and history of research in
...
graph_spending <- ggplot(dep_chomage, aes(x = time, y = `Public Spending`)) +
...
dep_by_country <- ggplot(dep_by_country, aes(x = time, y = `Public Spending`, color = Country)) +
  geom_line() +  
  geom_point() + 
...
print(dep_by_country)
```
Focusing solely on the G8 countries, we have assigned a color to each country, showing that the United States, France and Great Britain are the countries that spend the most on education. We can see that USA's public spending remains around 5,5 compared to Italy with 4,1. 
